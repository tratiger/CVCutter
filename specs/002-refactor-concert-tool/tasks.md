...
- [ ] T009 [P] Add processing-event envelope/retry-payload/immutability RED contract tests in `tests/contract/test_processing_event_contract.py`.
...
- [ ] T016 [P] Add operator-role normalization RED contract tests in `tests/contract/test_authorization_role_contract.py`.
...
- [ ] T035 [US2] Implement confidence scoring policy and classification output payload schema (`strategy`, scores, confidence_state, reason_codes, trace_context) in `src/cvcutter/domain/classification/confidence_policy.py`.
...
- [ ] T052 [P] [US4] Add external adapter contract RED tests including `fetch_form_responses` operation in `tests/contract/test_external_integration_adapter_contract.py`.
...
- [ ] T058 [US4] Implement approved adapter interfaces with pinned API inventory and Google Forms response fetch operation in `src/cvcutter/infrastructure/integrations/adapters.py`.
...
- [ ] T065 [US4] Implement plaintext credential config write/read and consent-gated enablement flow in `src/cvcutter/infrastructure/persistence/plaintext_credential_repository.py`.
...
T050 src/cvcutter/presentation/flet_app/views/segment_review_view.py
```
